# VoiceDock 詳細仕様書 v5.49

**DJI Mic 3 × ローカル文字起こし × ローカルLLM × Obsidian**

| 項目 | 内容 |
|---|---|
| 文書版 | **v5.49（生成経路と時刻を 1 か所に寄せる）** |
...
| R-4 | **whisper.cpp の CPU 実行速度** | 1 日分の処理が 8 時間を超えると翌日分に追いつけず遅延が累積する | **クローズ**（2026-09-14 に実測。`docs/POC.md` §12.5 / §12.6）。**16 時間 → 6.95 時間で Phase 2 を満たす**（カバレッジ 17.83%）。`large-v3-turbo-q5_0` のままでよい。**ただし余裕は 13% しかない** — 6 本中 2 本が単独では許容上限 `rtf 0.5` を割っており（最悪 0.724）、**素材によっては超える。**実データでの確認は #125（運用の中で） |
