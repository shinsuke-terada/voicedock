# VoiceDock 詳細仕様書 v5.23

**DJI Mic 3 × ローカル文字起こし × ローカルLLM × Obsidian**

| 項目 | 内容 |
|---|---|
| 文書版 | **v5.23（取り込み時間の内訳を実測する）** |
| 前身文書 | v5.22 / v5.21 / v5.20 / v5.19 / v5.18 / v5.17 / v5.16 / v5.15 / v5.14 / v5.13 / v5.12 / v5.11 / v5.10 / v5.9 / v5.8 / v5.7 / v5.6 / v5.5 / v5.4 / v5.3 / v5.2 / v5.1 / v5.0 / v4.6 / v4.5 / v4.4 / v4.3 / v4.2 / v4.1 / v4.0 / v3.4 / v3.3 / v3.2 / v3.1 / v3.0（git 履歴）、`docs/archive/VoiceDock_Docker_Implementation_Spec_v2.0.md`（方針書） |
| 作成日 | 2026-09-11 |
| 改訂日 | 2026-09-14 |
...
import:
  ...
  # **plist の StartInterval（300 秒）より十分大きくすること。**Helper は実行の最後に
  # heartbeat を書くので、次の書き込みまでの経過は 300 秒 + 実行時間になる。
...
| **DH-17** | **`config.yaml` の `import.helper_heartbeat_max_age_seconds` が、LaunchAgent の `StartInterval` より大きい**（等しくてはならない）。**Helper は実行の最後に heartbeat を書く**ので、次の書き込みまでの経過は `StartInterval + 実行時間` になる。等しいと**毎周期 stale と判定して取り込みを見送り**、§19.1 H-8 の healthcheck も境界で振れる。**この 2 つの数字は別のファイルに散っており、どちらも単体では正しく見える** | 致命的 |
