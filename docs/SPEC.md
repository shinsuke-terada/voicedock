# VoiceDock 詳細仕様書 v5.46

**DJI Mic 3 × ローカル文字起こし × ローカルLLM × Obsidian**

| 項目 | 内容 |
|---|---|
| 文書版 | **v5.46（削除結果の照合先を DB へ／Helper 停止で全処理を止めない）** |
...
def resolve_delete_targets(part: Recording) -> list[DevicePath]:
    ...
    引数は DB 行である（v5.8→v5.9 の変更 U-1）。PartCandidate は inbox 走査の型で
    status も source_path も持たない。§14.1 が見ている part は最初から DB 行である。
    """
