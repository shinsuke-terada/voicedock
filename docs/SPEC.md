...
| **結果の `request_id` が現在の要求と食い違う** | **結果を捨てる**（変更 BG-2）。古い試行のものである。要求が 1 件も無い場合も同じ（取り下げ後に届いた） |
