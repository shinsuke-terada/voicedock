...
> Phase 7 の前提（ND-01〜ND-31 と E2E-01〜E2E-12 の全件 PASS。**§21.2**）が揃うまでは、
...
|---|---|---|
| **コンテナ層**（ND-01〜ND-17、**ND-21、ND-30、ND-31**） | §14.1 が偽のとき **`queue/delete/` に要求が 1 件も書かれない**こと | pytest。偽 inbox と偽 state を与える |
...
| **Phase 7** | 自動削除 | **§20.4 の ND-01〜ND-31 が全件 PASS** かつ **E2E-01〜E2E-12 が全件 PASS** した後にのみ、三重ロック（§14.2）を解除する。**E2E-06 を運用の中で確認する扱いにしても、このゲートは緩めない**（#125） — **元音声を消す判断の根拠だからである。**運用で確認できてから Phase 7 へ進む |
...
| 22 | 保存失敗時に元音声を削除しない | §13.7, §14.1, ND-07〜ND-16 | ✅ ND-07〜16。**削除は三重ロックで止まったまま** |
