# VoiceDock 詳細仕様書 v5.51

**DJI Mic 3 × ローカル文字起こし × ローカルLLM × Obsidian**

| 項目 | 内容 |
|---|---|
| 文書版 | **v5.51（Raw ノートを見出しごとに 1 段落へ）** |
...
| V-34 | `obsidian.wiki.include_transcript` が false なら `obsidian.wiki.link_raw` が true | `CONFIG_INVALID_VALUE`（両方 false だと Daily ノートに本文もリンクも残らず、**§13.7 W-9 が永久に満たせない**。その日のノートは二度と `SAVED` にならない） |
