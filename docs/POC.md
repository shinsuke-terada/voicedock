...
### 4.2 `rw` なら削除できる（P0-8 の先行確認）

**実機では行わない**（#2 の非スコープ。P0-8 の本番確認は #38）。使い捨ての FAT32 ディスクイメージ上で確認した。
...
| P0-6 | **コンテナ起動後の接続が `/host-volumes` に現れる** | ✅ PASS | §3.1。取り外し 1 秒 / 再接続 9 秒。**ただし gRPC FUSE 上でのみ** |
| P0-7 | コンテナから WAV を読み取れる | ✅ PASS | §3.2。**ただし gRPC FUSE 上でのみ。VirtioFS では FAIL** |
| P0-8 | コンテナから WAV を削除できる（`rw`） | ✅ 先行確認 | §4.2。ディスクイメージ上。実機は #38 |
| P0-9 | non-root（uid 1000）で読めるか | ✅ PASS | §3.2。実機の WAV を全バイト読了 |
| P0-10〜P0-15 | — | ⬜ 未実施 | #3 |

**Phase 0 は通過していない。**P0-10〜P0-15 が #3 待ちであることに加え、
**§8 の設計変更により P0-6 / P0-7 / P0-8 の測定条件そのものが変わる**（コンテナはデバイスを直接見なくなる）。
設計変更後に #3 で測り直す。
